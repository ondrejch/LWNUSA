...
set out "study_hex_pitch.png"
set ytics nomirror
set y2tics 0.1
set xtics 0.1
set grid
set title "Hexagonal pitch, 225 rods"
set xlabel "pitch [cm]"
set ylabel "k_{eff}"
set y2label "maximum radius of hole between rods [cm]"

plot [3.95:5.35]"tab.dat" u 1:2:3 w e  tit "k_{eff}", "" w lp notit ls 1, "" u 1:(\$1/sqrt(3) - 1.655) axes x1y2 w l tit "max hole radius"
EOF
